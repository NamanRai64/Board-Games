...
import { StatusBanner, SessionStats, ResultModal } from '../components';
import { Ship, User, Skull, Waves } from 'lucide-react';
...
export default function Missionaries() {
  ...
  const updateState = useCallback((newState) => {
    setState(newState);
    const res = checkStatus(newState);
    if (res !== 'playing') {
      setStatus(res);
  ...
  const performAgentMove = useCallback(() => {
    ...
    if (res.chosen && res.chosen.score > -50) {
      ...
      const nextWay = state.boat === 'left' ? 'right' : 'left';
      const mult = state.boat === 'left' ? -1 : 1;
      const newState = { ml: state.ml + mult * m, cl: state.cl + mult * c, mr: state.mr - mult * m, cr: state.cr - mult * c, boat: nextWay };
      updateState(newState);
    } else { setStatus('lose'); setIsAuto(false); }
  }, [calculateAgentMove, status, state.boat, state.ml, state.cl, state.mr, state.cr, updateState]);
